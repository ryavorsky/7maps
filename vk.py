...
fout.write('id,name,sex,bdate,age,city,count_posts,count_reposts\n')
users=[]
for i in range (len(publics_list['public_ids'])):
	...
	while True:
		url = 'https://api.vk.com/method/groups.getMembers?fields=name,sex,bdate,city&access_token=' + access_token + '&count=1000&offset=' + str(offset) + '&group_id=' + str(publics_list['public_ids'][i])
		members = urllib2.urlopen(url).read()
		members = json.loads(members)
		sleep(0.5)
		for member in members['response']['users']:
			if not (member['uid'] in users):
				...
				count_original = 0
				count_reposts = 0
				while (wall_count == 100 and date_diff.days < 366):
					wall = 'https://api.vk.com/method/wall.get?access_token=' + access_token + '&offset=' + str(wall_offset) + '&count=100&owner=' + str(member['uid'])
				...
				fout.write('%d,%s,%s,%s,%s,%s,%d,%d\n' % (member['uid'], member['first_name'].encode("utf-8") + ' ' + member['last_name'].encode("utf-8"), sex[member['sex']-1], bdate, str(age), city, count_original, count_reposts))
